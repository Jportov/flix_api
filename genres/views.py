import json
from django.views.decorators.csrf import csrf_exempt
from django.http import JsonResponse
from django.shortcuts import get_object_or_404
from genres.models import Genre

@csrf_exempt
def genre_create_view(request):
    if request.method == 'GET':
        genres = Genre.objects.all()
        data = [{'id': genre.id, 'name': genre.name, 'description': genre.description} for genre in genres]
        return JsonResponse(data, safe=False)
    elif request.method == 'POST':
        data = json.loads(request.body.decode('utf-8'))
        new_genre = Genre(name=data['name'], description=data.get('description', ''))
        new_genre.save()
        return JsonResponse({'id': new_genre.id, 'name': new_genre.name, 'description': new_genre.description}, status=201)


@csrf_exempt
def genre_detail_view(request, pk):
    genre = get_object_or_404(Genre, pk=pk)

    if request.method == 'GET':
        data = {'id': genre.id, 'name': genre.name, 'description': genre.description}
        return JsonResponse(data)
    elif request.method == 'PUT':
        data = json.loads(request.body.decode('utf-8'))
        genre.name = data.get('name', genre.name)
        genre.description = data.get('description', genre.description)
        genre.save()
        return JsonResponse({'id': genre.id, 'name': genre.name, 'description': genre.description})
    elif request.method == 'DELETE':
        genre.delete()
        return JsonResponse({'message': 'Genre deleted successfully'}, status=204)